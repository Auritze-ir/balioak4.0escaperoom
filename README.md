...
Repositorio hau GitHub Pages bidez argitaratuta badago, sartu hemen:

```
https://<zure-erabiltzailea>.github.io/<repo-izena>/balioak-hub.html
```

> Aldatu `balioak-hub.html` orriaren `<head>`-eko esteka eta gainerako `href`ak ez dira aldatu behar — fitxategi guztiak **karpeta berean** egon behar dute, izenez estekatzen baitira.
...
2. Kiosk moduan abiarazteko (gomendatuena, ikasleek nabigatzaile-barra ez ikusteko):
   ```
   chrome.exe --kiosk --touch-events "https://<zure-erabiltzailea>.github.io/<repo-izena>/balioak-hub.html"
   ```
   Edo lokalki badago: `--kiosk --touch-events "C:\bidea\balioak-hub.html"`
...
4. Sarrera-bideoak (hub-ean) **internet konexioa** behar du (YouTube). Konexiorik ez badago, ordeztu `balioak-hub.html`-eko `<iframe>` bideo lokal batekin.

## 📁 Egitura

| Fitxategia | Deskribapena |
|---|---|
| `balioak-hub.html` | Gida nagusia (urratsez urrats): sarrera-bideoa, 4 gelen bidalketa (kokapenekin), azken kandadua |
...
1. Irakasleak `balioak-hub.html` irekitzen du — **urratsez urrats gida** da: sarrera-bideoa, gero gela bakoitza banan-banan.
